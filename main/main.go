...
// Create tool registry
	...
	// Create agent
	ag := agent.New(p, tr, sessions)

	// Handle --init-config flag
	if *initConfig {
		// Check for conflicting flags (anything except --verbose)
		if *prompt != "" || *clear {
...
// printResponse prints the response with formatting including header, time, and stats
func printResponse(ctx context.Context, response string, tokens map[string]int, durationMs float64) {
	// Get current time
	now := time.Now()
	timestamp := now.Format("15:04:05")

	// Print header line with time and stats (gray foreground, no background)
	fmt.Printf("\n\033[90m%s | ", timestamp)
	printTokenStats(tokens, durationMs)
	fmt.Println("\033[0m")

	// Print response content in cyan foreground
	fmt.Printf("\033[36m%s\033[0m\n\n", response)
}

// printTokenStats prints token usage statistics
func printTokenStats(tokens map[string]int, durationMs float64) {
	// Only print if we have token info
	if tokens == nil || len(tokens) == 0 {
		return
	}

	// Print tokens on same line
	var tokenParts []string
	if v, ok := tokens["prompt"]; ok {
		tokenParts = append(tokenParts, fmt.Sprintf("prompt=%d", v))
	}
	if v, ok := tokens["completion"]; ok {
		tokenParts = append(tokenParts, fmt.Sprintf("completion=%d", v))
	}
	if v, ok := tokens["total"]; ok {
		tokenParts = append(tokenParts, fmt.Sprintf("total=%d", v))
	}

	// Calculate tokens per second
	tokPerSec := 0.0
	if durationMs > 0 {
		if v, ok := tokens["total"]; ok && v > 0 {
			tokPerSec = (float64(v) / durationMs) * 1000
		}
	}

	// Join all stats on one line
	if tokPerSec > 0 && len(tokenParts) > 0 {
		fmt.Printf("%s | speed=%.1f tok/s", joinStrings(tokenParts), tokPerSec)
	} else if len(tokenParts) > 0 {
		fmt.Printf("%s", joinStrings(tokenParts))
	}
}
